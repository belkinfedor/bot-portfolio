...
"""
    ...
    def update_skills(self, param, data):
        sql = f"""UPDATE skills SET {param} = ? WHERE skill_name = ? AND user_id = ?"""
        self.__executemany(sql, [data])

    def update_status(self, param, data):
        sql = f"""UPDATE status SET {param} = ? WHERE status_name = ? AND user_id = ?"""
        self.__executemany(sql, [data]) 
