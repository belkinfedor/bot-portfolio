...
def start_command(message):
    bot.send_message(message.chat.id, """/new_project - добавить проект
                     /skills - выбрать навык проекту 🧐
                     /projects - показать все проекты 🙌
                     /delite - удалить проект ⛔
                     /update - изменить проект ♾️""")
    info(message)
...
def addtask_command(message):
    bot.send_message(message.chat.id, "Введите название проекта #️⃣:")
...
def name_project(message):
    ...
    data = [user_id, name]
    bot.send_message(message.chat.id, "Введите ссылку на проект *️⃣")
    bot.register_next_step_handler(message, link_project, data=data)

def link_project(message, data):
    data.append(message.text)
    statuses = [x[0] for x in manager.get_statuses()] 
    bot.send_message(message.chat.id, "Введите текущий статус проекта ⏹️", reply_markup=gen_markup(statuses))
